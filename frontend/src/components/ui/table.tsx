...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr className={cn("border-b transition-colors hover:bg-slate-50", className)} ref={ref} {...props} />
  )
);
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      className={cn("h-12 px-4 text-left align-middle text-xs font-medium uppercase text-slate-500", className)}
      ref={ref}
      {...props}
    />
  )
