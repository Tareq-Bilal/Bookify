...
public sealed class UsersTests(IntegrationTestWebAppFactory factory) : BaseIntegrationTest(factory)
{
    ...
    [Fact]
    public async Task GetIdByCredentials_Should_ReturnUserId()
    {
        // Arrange
        string email = UniqueEmail();
        ...
        Guid returnedUserId = await response.Content.ReadFromJsonAsync<Guid>();
        returnedUserId.ShouldBe(userId);
    }

    [Fact]
    public async Task GetIdByCredentials_Should_ReturnProblem_WhenPasswordIsInvalid()
    {
        // Arrange
        string email = UniqueEmail();
        await RegisterUserAsync(email);
